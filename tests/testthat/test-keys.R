test_that("filter_key_range() works", {
  ...
  expect_equal(
    filter_key_range(keys_data, "A1", "e")$white$name,
    c("A1", "B1", "C", "D", "E", "F", "G", "A", "B", "c", "d", "e")
  )
  expect_equal(
    filter_key_range(keys_data, "A1", "e")$black$name_sharp,
    c("A#1", "C#", "D#", "F#", "G#", "A#", "c#", "d#")
  )
  expect_equal(
    filter_key_range(keys_data, "A1", "e")$black$name_flat,
...
test_that("filter_key_range() error messages work", {
  expect_error(filter_key_range(keys_data, "A3", "c5"), "'A3' is not a valid lower key")
  expect_error(filter_key_range(keys_data, "A1", "e5"), "'e5' is not a valid upper key")
  expect_error(filter_key_range(keys_data, "c3", "c3"), "lower must be below upper")
  expect_error(filter_key_range(keys_data, "c3", "e2"), "lower must be below upper")
})
